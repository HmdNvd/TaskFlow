...
import { useChatNotifications } from '@/services/chatNotifications'
...
export const Header: React.FC<HeaderProps> = ({ onOpenMobileMenu }) => {
  ...
  const unreadCount = unreadConversations.reduce((total, item) => total + item.count, 0)
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-border/80 bg-card/90 px-4 backdrop-blur-md sm:px-6">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        {/* Notification Bell / unread chat messages */}
        <div className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -right-1 -top-1 min-w-4 h-4 rounded-full bg-primary px-1 text-[9px] font-bold leading-4 text-primary-foreground ring-2 ring-card">
          ...
          {notificationOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-xl border border-border/80 bg-card/95 shadow-xl backdrop-blur-md">
                ...
                <div className="divide-y divide-border/40">
                  {unreadConversations.map((conversation) => (
                    <button
                      ...
                      type="button"
                      className="flex w-full items-center gap-3 px-3 py-3 text-left hover:bg-muted/60 transition-colors"
                      onClick={async () => {
                        try {
                          const response = await usersApi.getById(String(conversation.userId))
                          setChatTarget({
                          ...
                          setChatTarget({ id: conversation.userId, name: 'Conversation' })
                          setNotificationOpen(false)
                        }
                      }}
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-[10px] text-muted-foreground">
                          {conversation.count} unread message{conversation.count === 1 ? '' : 's'}
                        </span>
                      </span>
